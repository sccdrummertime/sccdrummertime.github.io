...
Platform notes: on iOS the ring/silent switch mutes plain Web Audio, so there the engine
routes its output through a MediaStream into a hidden `<audio>` element — the clicks are
then media playback (like a music app) and stay audible on silent. LED/torch flash was
